	...
	def update(self):
		...
		for player in players:
			if player.state == player.STATE_ALIVE and self.rect.colliderect(player.rect):
				if player.bulletImpact(self.owner == self.OWNER_ENEMY, self.damage, self.owner_class):
					self.destroy()
					return
...
class Game():
	...
	def drawIntroScreen(self, put_on_surface = True):
		...
		if pygame.font.get_init():
			...
			screen.blit(self.font.render("2 PLAYERS", True, pygame.Color('white')), [165, 300])
		...
		if self.nr_of_players == 1:
			...
			screen.blit(self.player_image, [125, 300])
					...
					for player in players:
						if player.state == player.STATE_ALIVE:
							try:
								...
								if index == 0:
									if player.fire(True) and play_sounds:
										#sounds["fire"].play()
										pass
								elif index == 1:
